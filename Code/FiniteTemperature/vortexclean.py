...
class GPETimeEv():
    ...
    def setbox2(self):
        ...
        '''
        #self.height = self.kmax**2/2*3 # 20% larger
        self.height = 5
        V = 0
        for i in range(self.dim):
            #previously height was 5
            V += self.height * np.array(np.power(np.e, -2*(self.xi[i] - np.ones_like(self.xi[i])*self.L/2)**2) + np.power(np.e, -2*(self.xi[i] + np.ones_like(self.xi[i])*self.L/2)**2))
