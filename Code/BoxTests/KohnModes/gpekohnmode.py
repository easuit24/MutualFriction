...
class GPETimeEv():
    ...
    def __init__(self, L = 50, npoints = 2**9, dim = 1, tol = 10e-10, Nfactor = 1000, runDyn = True): 
    ...
    def setVsho(self, w = 0.5, shift = 0): 
        '''
        ...
        shift - shift of the potential energy curve (effect of shifting the cloud)
        '''
        self.w = w
        Vs = 0
        for i in range(self.dim): 
            Vs += 0.5 * w**2 * (self.xi[i] - shift)**2 
    ...
    def simulateimag(self, Nsteps = 500): 
        '''
        ...
        For Kohn's theorem, we propagate in imaginary time in a SHO potential 
        '''
        ...
        for i in range(Nsteps): 
            ...
            potUim = np.power(np.e, -(self.Vs + self.g_sho  * np.abs(self.psi)**2 - 1) *self.dt)
    ...
    def simulatebreather(self):
        ...
        self.snapshots = [self.psi] 
        self.dynpsi = self.psi.copy() # set dynamic psi to evolve in time
        self.Vs = self.setVsho(shift = 7) 
        
        for i in range(30000): # TODO: change this back to 10000 later
