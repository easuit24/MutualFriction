...
class GPETimeEv():
    ...
    def calcEnergy(self): 
        ...
        #temp = self.r**2*np.abs(self.dynpsi)**2*self.dr
        #print(np.shape(temp))
        #Ep = self.w**2 / 2 * np.cumsum(np.cumsum(self.r**2*np.abs(self.dynpsi)**2*self.dr)*self.dr)[-1]
        ...
        #self.Ep_arr2.append(Ep2) 


    def simulateimag2(self, Nsteps = 5000): 
        '''
    ...
    def simulatebreather(self):
        ...
        for i in range(5000): # TODO: change this back to 10000 later
    ...
    def animatepsi2d_save(self):
        fig, ax = plt.subplots() 
        data = plt.imshow(np.abs(self.snapshots[0]), extent = [-self.L, self.L, -self.L, self.L],cmap = plt.cm.hot)
