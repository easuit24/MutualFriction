...
class GPETimeEv():
    ...
    def setxgrid(self): 
        ...
        for i in range(self.dim): 
            ...
            box = np.linspace(-self.L/2,(self.npoints-1)*self.dx-self.L/2,self.npoints)
            winL = np.linspace(-self.winL/2, -self.L/2, self.npoints/self.winMult) # assign the ratio of the window points to the box points 
            winR = np.linspace(self.L/2, self.winL/2, self.npoints/self.winMult)
            axis = np.hstack((winL, box, winR))
            axes.append(axis)
    ...
    def setpgrid(self): 
        ...
        for i in range(self.dim): 
            ...
            #box = np.linspace(-np.pi/self.dx, (self.npoints - 1) * (2*np.pi)/self.L - (np.pi/self.dx), self.npoints)
    ...
    def trackVortices(self): 
        ...
        if self.vort_coords[0][0] == 0 or self.vort_coords[0][1] == 0 or self.vort_coords[0][0] == len(self.searchArea)-1 or self.vort_coords[0][1] == len(self.searchArea[0])-1: 
            ...
            x_diff = self.centers[0][0] - self.vort_coords[0][0] # define the center to be around the initial vortex center   
            y_diff = self.centers[0][1] - self.vort_coords[0][1]
        ...
        if self.vort_coords2[0][0] == 0 or self.vort_coords2[0][1] == 0 or self.vort_coords2[0][0] == len(self.searchArea2)-1 or self.vort_coords2[0][1] == len(self.searchArea2[0])-1: 
            ...
            x_diff = self.centers[1][0] - self.vort_coords2[0][0] # define the center to be around the initial vortex center  
            y_diff = self.centers[1][1] - self.vort_coords2[0][1]
