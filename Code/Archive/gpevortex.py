...
class GPETimeEv():
    ...
    #     axis = self.xi[0][0] 
    #     start_ind = self.winL//2/self.dx - self.neighborhoodHalfLength
    #     end_ind = start_ind + 2*self.neighborhoodHalfLength 
    #     self.neighborhoodAxis = axis[int(start_ind):int(end_ind)+1]
    #     self.searchArea = np.abs(self.dynpsi[int(start_ind):int(end_ind)+1,int(start_ind):int(end_ind)+1]) # assuming square neighborhood grid 

    #     ## TODO next: find the minimums within the neighborhood set 
    #     axes_neighb = [] 
    #     for i in range(self.dim): 

    #         axes_neighb.append(self.neighborhoodAxis)
    #     neighbaxes_arr = np.array(axes_neighb) 
    #     self.xi_neighb = np.meshgrid(*neighbaxes_arr) 
    ...
    #     print("Minima at: ", minima)

    #     curr_vlocs = np.where(1 == minima)
    #     for i in range(len(curr_vlocs)):
    #         if curr_vlocs[i][0] == 0 or curr_vlocs[i][0] == len(self.searchArea)-1 or curr_vlocs[i][1] == 0 or curr_vlocs[i][1] == len(self.searchArea)-1:
    #             # TODO: move search area centered around the percieved minimum 
    #             return 
    #         else: 
    #             self.vortex_locs.append(curr_vlocs)
