...
class FiniteTempGPE():
    ...
    def genGroundState(self): 
        ...
        if self.vortex: 
            ...
            self.gpeobj = gpeb(L = self.L, dtcoef = self.dtcoef, dim = self.dim, numImagSteps=self.numImagSteps, runDyn = False, winMult=self.winMult)
    ...
    def realpropagate(self, wf, numSteps): 
        ...
        for i in range(numSteps): 
            ...
            psiFTold = dstn(dynpsi)
            psiFTnew = psiFTold * kinU 
            psiinterim = idstn(psiFTnew)
    ...
    def animatepsi2d(self, filename):
        ...
        fig, ax = plt.subplots() 
        data = plt.imshow(np.abs(self.snaps[0]), extent = [-self.winL/2, self.winL/2, -self.winL/2, self.winL/2],cmap = plt.cm.hot)
        ...
        def animate(i): 
            data.set_data(np.abs(self.snaps[i]))
