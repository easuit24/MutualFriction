...
class GPETimeEv():
    ...
    def __init__(self, L = 20, npoints = 2**9, dim = 1, numVort = 10, spawnType = 'pair', tol = 10e-10, Nfactor = 1000, numImagSteps = 2000, numRealSteps = 10000, antiV = False, dist = 2, runDyn = True): 
    ...
    def simulateimag(self): 
       ...
       #kinUim = np.power(np.e, -self.k2*self.dt)
        ...
        for i in range(self.numImagSteps): 
            ...
            psiFTold = np.fft.fftshift(np.fft.fft2(self.psi)) 
    ...
    def simulatevortex(self):
       ...
       # kinU = np.power(np.e, -(1.0j )*(self.k2)*self.dt)
        ...
        #self.dynpsi = self.vortexFunc(self.psi.copy()) # set dynamic psi to evolve in time
